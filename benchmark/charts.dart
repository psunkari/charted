...
List<Map<String, int>> generateChartData(int rowCount, int measureColumnCount,
...
List<ChartColumnSpec> generateColumnSpecs(List<Map<String, int>> data) => data[
        0]
    .keys
    .map((key) => key == "domain"
        ? new ChartColumnSpec(label: key, type: ChartColumnSpec.TYPE_TIMESTAMP)
        : new ChartColumnSpec(label: key))
    .toList();
...
main() {
  var generatedValues = generateChartData(30, 2);
  var columnSpecs = generateColumnSpecs(generatedValues);
  var rowData = generatedValues.map((value) => value.values.toList()).toList();
  document
      .querySelector('#benchmark-trigger')
      .onClick
      .listen((_) => line(columnSpecs, rowData));
}
